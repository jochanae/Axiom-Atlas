...
router.get("/projects/:id/state", async (req, res): Promise<void> => {
  ...
  const [project] = await db
    .select({
      ...
      surfaceMode: projectsTable.surfaceMode,
      shape: projectsTable.shape,
      workingTitle: projectsTable.workingTitle,
  ...
  res.json({
    project: {
      ...
      surfaceMode: project.surfaceMode,
      shape: project.shape,
      workingTitle: project.workingTitle,
      committedAt: project.committedAt ? project.committedAt.toISOString() : null,
