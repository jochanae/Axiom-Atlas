...
export default function MasterMap() {
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([
      fetch(`${BASE_URL}/api/projects/nexus`, { credentials: "include" }).then(r => r.ok ? r.json() : null),
      fetch(`${BASE_URL}/api/projects`, { credentials: "include" }).then(r => r.ok ? r.json() : []),
    ]).then(([nexus, all]) => {
      setNexusProject(nexus);
      setProjects((all as Project[]).filter((p: Project) => !p.isNexus));
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ fontSize: 10, fontFamily: "var(--app-font-mono)", color: "rgba(120,113,108,0.5)", letterSpacing: "0.08em" }}>
      ...
      >
        ...
        <div style={{
          ...
          <svg style={{ position: "absolute", inset: 0, width: "100%", height: "100%", overflow: "visible" }}>
            ...
            {/* Edges from Nexium to each project */}
            {projects.map((p, i) => {
          ...
          {!loading && projects.length === 0 && (
            <div style={{
              position: "absolute",
              left: CENTER_X - 100, top: CENTER_Y + 80,
        ...
        {/* Node action card — floats in screen space */}
        {focusedProject && (() => {
          ...
          return (
            ...
            >
              ...
              <div style={{ fontSize: 12, fontWeight: 600, color: "rgba(231,229,228,0.9)", marginBottom: 2, paddingRight: 18, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                {focusedProject.name}
              </div>
              <div style={{ fontSize: 9.5, color: "rgba(120,113,108,0.5)", fontFamily: "var(--app-font-mono)", marginBottom: 11 }}>
                {activityLabel(focusedProject.updatedAt)}
              </div>

              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                <button
                  onClick={() => navigateToProject(focusedProject.id)}
                  style={{
                ...
                >
                  <svg width="11" height="11" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M8 2H3a1 1 0 00-1 1v10a1 1 0 001 1h10a1 1 0 001-1V8" />
                    <path d="M10 2h4v4M14 2L8 8" />
                  </svg>
                  Open Workspace
                </button>
                <button
                  onClick={() => navigateToProject(focusedProject.id, true)}
                  style={{
                    width: "100%", padding: "7px 10px", borderRadius: 7,
