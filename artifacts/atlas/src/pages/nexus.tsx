    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100dvh", background: "var(--atlas-bg)", fontFamily: "var(--app-font-sans)", overflow: "hidden" }}>
      ...
      <header style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 7, flex: 1, minWidth: 0 }}>
          ...
          <span style={{ fontSize: 13, fontWeight: 600, color: "var(--atlas-gold)", letterSpacing: "0.01em" }}>Nexium</span>
          <span style={{ fontSize: 10, color: "var(--atlas-muted)", fontFamily: "var(--app-font-mono)", opacity: 0.55, letterSpacing: "0.06em", display: isMobile ? "none" : undefined }}>
        ...
        {!isMobile && (
          <button
            title="Stamp a Nexium decision into a specific project's ledger"
      ...
      <ProjectsDrawer
        ...
        projects={(allProjects ?? []).map(p => ({ id: p.id, name: p.name, description: p.description, latestSnapshotScore: p.latestSnapshotScore ?? null }))}
                      ...
                      try {
                        await createEntry.mutateAsync({
                          ...
                          data: {
                            ...
                            status: "committed", severity: "committed", mode: "think",
                          }
                        });
                        queryClient.invalidateQueries({ queryKey: getListProjectsQueryKey() });
                        setCommitDone(true);
                        setTimeout(() => setShowCommit(false), 1600);
                      } catch { /* noop */ } finally { setCommitPending(false); }
                    }}
                    style={{
                      padding: "7px 16px", borderRadius: 8,
                      background: commitTargetId && commitTitle.trim() ? "var(--atlas-ember)" : "rgba(146,64,14,0.25)",
                      border: "none", color: commitTargetId && commitTitle.trim() ? "#fff" : "rgba(255,255,255,0.3)",
                      cursor: commitTargetId && commitTitle.trim() ? "pointer" : "not-allowed",
